...
{
    randomSlice = FALSE;       // Initialize to disable pseudo-random time slicing.
    debugUserProg = FALSE;     // Initialize to disable single-step user program debugging.
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
            ...
            debugUserProg = TRUE;                   // Enable single-step user program debugging.
		} else if (strcmp(argv[i], "-e") == 0) {
        	execfile[++execfileNum]= argv[++i];    // Store executable file names.
			cout << execfile[execfileNum] << "\n";
		} else if (strcmp(argv[i], "-ci") == 0) {
	    	ASSERT(i + 1 < argc);
	    	consoleIn = argv[i + 1];         // Set console input file.
	    	i++;
		} else if (strcmp(argv[i], "-co") == 0) {
	    	ASSERT(i + 1 < argc);
	    	consoleOut = argv[i + 1];       // Set console output file.
	    	i++;
#ifndef FILESYS_STUB
		} else if (strcmp(argv[i], "-f") == 0) {
	    	formatFlag = TRUE;  // Enable disk format flag.
#endif
        } else if (strcmp(argv[i], "-n") == 0) {
            ASSERT(i + 1 < argc);   // next argument is float
            reliability = atof(argv[i + 1]); // Set network reliability.
            i++;
        } else if (strcmp(argv[i], "-m") == 0) {
            ASSERT(i + 1 < argc);   // next argument is int
            hostName = atoi(argv[i + 1]); // Set machine ID.
            i++;
...
{
    // cout<<"Thread exec "<<name<<" priority "<<priority<<"\n";
